...
class ReActAgentStrategy(AgentStrategy):
    ...
    def _system_prompt_message(self) -> SystemPromptMessage:
        ...
        system_prompt = (
            ...
            .replace(
                "{{tools}}",
                json.dumps([tool.model_dump(mode="json") for tool in self._prompt_messages_tools]),
            )
            .replace(
                "{{tool_names}}",
    ...
    def _invoke(self, parameters: dict[str, Any]) -> Generator[AgentInvokeMessage]:
        ...
        stop = react_params.model.completion_params.get("stop", []) if react_params.model.completion_params else []
        if "Observation" not in stop and model.provider not in ignore_observation_providers:
        ...
        react_params.model.completion_params = react_params.model.completion_params or {}
        ...
        while run_agent_state and iteration_step <= max_iteration_steps:
            ...
            prompt_messages = self._organize_prompt_messages(agent_scratchpad, self.query)
            ...
            scratchpad.thought = (
                scratchpad.thought.strip() if scratchpad.thought else "I am thinking about how to help you"
            )
            ...
            action_dict = scratchpad.action.to_dict() if scratchpad.action else {"action": scratchpad.agent_response}
            ...
            yield self.finish_log_message(
                ...
                metadata={
                    ...
                    LogMetadata.PROVIDER: model.provider,
                    LogMetadata.TOTAL_PRICE: usage_dict["usage"].total_price if usage_dict["usage"] else 0,
                    LogMetadata.CURRENCY: usage_dict["usage"].currency if usage_dict["usage"] else "",
                    LogMetadata.TOTAL_TOKENS: usage_dict["usage"].total_tokens if usage_dict["usage"] else 0,
                },
            )
                ...
                if scratchpad.action.action_name.lower() == "final answer":
                    ...
                    tool_call_log = self.create_log_message(
                        ...
                        metadata={
                            ...
                            LogMetadata.PROVIDER: tool_instances[tool_name].identity.provider
                    ...
                    yield tool_call_log
                    tool_invoke_response, tool_invoke_parameters, additional_messages = self._handle_invoke_action(
            ...
            yield self.finish_log_message(
                log=round_log,
                data={
                    "action_name": scratchpad.action.action_name if scratchpad.action else "",
                    "action_input": scratchpad.action.action_input if scratchpad.action else "",
    ...
    def _organize_prompt_messages(self, agent_scratchpad: list, query: str) -> list[PromptMessage]:
        """
        Organize
        """
